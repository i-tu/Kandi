...
% http://dspace.cc.tut.fi/dpub/bitstream/handle/123456789/6349/rauhamaki.pdf?sequence=3
...
FBench on avoimen lähdekoodin standardimukainen ohjelmisto, joka kuitenkin kärsii samoista puutteista kuin FBDK. 4DIAC PROFACTORin kehittämä 4DIAC tarjoaa standardeja noudattavan funktioblokkieditorin. Se ei kuitenkaan mahdollista blokkien välisten lankojen reitittämistä. 
...
Vaadimme lisäksi, että se on \emph{kunnollinen} osion 3.1 mukaisesti.
...
Kun halutaankin mahdollisimman lyhyitä kerrostuksia, on suotavaa käyttää multiprosessoriajastusongelman ratkaisemiseksi kehitettyä Coffmann-Graham -algoritmia. \cite{RefWorks:59} Käyttöön tarkoitetun graafin pituuden rajoittaminen ei kuitenkaan ole tavoitteena, 

Työssä käytetään Gansner ym. esittäämää lineaariseen ohjemointiin perustuvaa lineaarisen ohjelmoinnin menetelmää, \emph{Network Simplex}- algoritmia. Sen aikakompleksisuutta ei ole pystytty osoittamaan, mutta se on käytännössä osoittautunut suoritusajaltaan nopeaksi.
...
Kun mahdollinen virityspuu on laadittu, käytetään hyväksi virityspuiden ominaisuutta, että jos jokin sen lanka poistetaan, tuotetaan kaksi toisistaan erillistä puuta. Jokaiselle langalle määritellään \emph{leikkausarvo}. Leikkausarvolla tarkoitetaan kaikkien  
