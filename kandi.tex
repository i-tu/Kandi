...
Työssä käytetty algoritmi seuraa Klauske et. al. kehittämää  datavirtauskaavioiden rajoitukset huomioon ottavaa lähestymistapaa. Se yhdistelee useita graafinpiirron tuloksia.

Se koostuu viidestä vaiheesta:
\begin{enumerate}
  \item Syklien poisto. Seuraa Eades et al.\cite{RefWorks:48}
  \item Kerrostus. Seuraa Gansner et. al., minimoi lankojen pituuen \cite{RefWorks:28}
  \item Risteysten vähentäminen noodeja järjestämällä seuraa \cite{RefWorks:9}
  \item Poikittainen asettelu Seuraa Sanderin lineaaristen segmenttien menetelmää. \cite{RefWorks:49}
  \item Lankojen ortogonaalinen reititys. Seuraa Sanderin monilankareititysalgoritmia. \cite{RefWorks:17}
\end{enumerate}
...
Syklejä ei voi kuitenkaan poistaa. Tämän vuoksi syklit rikotaan kääntämällä yksi sen langoista. 
Syklillisen graafin muuntaminen syklittömäksi tavalla joka minimoi käännettävien lankojen määrän on np-vaikea.

Käsittelemämme ongelma kuitenkin sisältää esitietona jokaisen puun ensimmäisenä suoritettavat noodit, jolloin graafin suuntaamisesta tuleekin triviaali toimenpide. [?]
...
Kerrostetuksen graafin tulee myös olla kompakti.
...
Siinä missä vaihe 3 vaikuttaa lankojen risteysten määrään, vaihe 4 vaikuttaa lankojen kulmien määrään.
...
Poikittaiseen asetteluun, kutn

		\subsection{Lankojen reititys}

Lankojen

Keskeneräinen: Tässä selitetään Sanderin "Layout of directed hypergraphs with orthogonal hyperedges" \cite{RefWorks:17}
